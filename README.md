...
The bot needs the **Connect**, **Speak**, **View Channel**, **Read Message History**, **Send Messages**, and **Embed Links** permissions. The invite link generated by the dashboard includes these permissions and uses the full OAuth code-grant flow, so it works even when **Require OAuth2 Code Grant** is enabled in the Discord Developer Portal. The Docker image includes yt-dlp's YouTube EJS support and enables its bundled Node 22 runtime automatically. Slash commands are synchronized per-server whenever the bot starts, including after an update, with automatic retries for temporary Discord API failures. Per-server commands update immediately, and the bot removes the old global command copy so Discord does not serve a stale version.

## Using karaoke

In Discord, use `/play song:...` with a song title, artist, YouTube/SoundCloud URL, Spotify playlist URL, or regular playlist URL. Playlist links are expanded in order and every playable track is added to the queue. Spotify importing uses the Spotify Web API for playlist metadata only, then matches each song to an existing YouTube or SoundCloud source for playback; it does not download or stream Spotify audio. Set `SPOTIFY_CLIENT_ID`, `SPOTIFY_CLIENT_SECRET`, and optionally `SPOTIFY_MARKET` in `.env` to enable it. The bot joins the requester's current voice channel, finds audio with `yt-dlp`, normalizes source loudness with FFmpeg, and searches LRCLIB for synchronized `.lrc` lyrics. When timed lyrics are unavailable but complete lyrics exist, the bot posts the full lyrics instead and labels them as unsynchronized. Lyrics are posted directly into Discord's built-in chat for that voice channel, so there is no separate lyrics-channel choice. The bot stays locked to its active voice channel while playing; use `/leave` before intentionally moving it. Use `/queue`, `/skip`, `/pause`, `/resume`, `/stop`, `/join`, and `/leave` to control the room. Use `/volume level:70` to change the gain of the song already playing, and `/instrumental` to reduce the lead vocal.
...
The dashboard has the same controls. The selected server name remains visible in the workspace header. Search by name in the YouTube or SoundCloud tab, or paste a YouTube, SoundCloud, or Spotify playlist URL directly. Playlist URLs are detected and opened in an import preview showing every resolved song; you can shuffle that preview before adding it. The add button changes between Play track, Add track, and Add playlist based on the current input and playback state. Only the active provider is searched, so results from one service cannot crowd out results from the other. Results are labelled by provider. Its Play and Join buttons use the logged-in user's current Discord voice channel; the Join button also accepts a channel ID when the browser session cannot see the voice state. The active room cannot be taken over from another voice channel. The person who queued the current track can skip it, other people can skip it after that requester leaves the room, and members with the Discord Administrator permission can always skip, including from outside the active voice channel. Upcoming tracks can be dragged into a new position, removed individually, or cleared without stopping the current song. The playback panel has a live volume slider that applies to the song currently playing, while the default volume under server preferences sets where each new session starts.

If `BOT_OWNER_ID` is set to the bot owner's Discord user ID, that account gets a private usage panel after logging in. It lists every server where the bot is installed, its current active/idle state and voice-channel activity, tracked unique users, web API requests, Discord slash-command requests, queue size, and last use. Usage aggregates are stored in the local database; message content and IP addresses are not recorded.
...
Set `BOT_OWNER_ID` to the bot owner's Discord user ID to enable the private owner usage panel. This is different from the bot/application ID and is optional.
