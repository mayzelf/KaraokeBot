...
The bot needs the **Connect**, **Speak**, **View Channel**, **Read Message History**, **Send Messages**, and **Embed Links** permissions. The invite link generated by the dashboard includes these permissions and uses the full OAuth code-grant flow, so it works even when **Require OAuth2 Code Grant** is enabled in the Discord Developer Portal. The Docker image includes yt-dlp's YouTube EJS support and enables its bundled Node 22 runtime automatically. Slash commands are bulk-synchronized globally whenever the bot starts, including after an update, with automatic retries for temporary Discord API failures. Discord can take a short time to show global command changes.
